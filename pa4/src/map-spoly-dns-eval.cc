//========================================================================
// map-spoly-dns-eval.cc
//========================================================================
// This program evalutes the performance of Map<Key,Value> by running for
...
int main( void )
{
  ...
  for ( size_t i = 0; i < ntrials; i++ ) {
...

    {
      ...
      Map< ShortString, IpAddress > my_map;

      for ( size_t j = 0; j < size_data; j++ )
        my_map.insert( url_data_arr[ j ], ip_data_arr[ j ] );
  ...
  for ( size_t i = 0; i < size_search; i++ ) {
    if ( result[ i ] != ref[ i ] ) {
      std::cout << "Error: Search for "     << url_search_arr[ i ]
